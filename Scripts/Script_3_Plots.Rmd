...
date: "2026-02-02"
---
###run Script_3_Stdeconvolve_lesion.rmd file before running this script
...
```{r sample Qc spots retained per patient after clean counts}
# Get spot counts from lesion-only object
before_lesion_qc <- table(integrated_Eth_cohort$sample_id[
  integrated_Eth_cohort$group == "lesion"])

after_lesion_qc <- table(integrated_Eth_cohort_filteredL$sample_id)

# Combine into comparison dataframe
lesion_qc_df <- data.frame(
  sample_id = names(before_lesion_qc),
  before_QC = as.numeric(before_lesion_qc),
  after_QC  = as.numeric(after_lesion_qc)
) %>%
  mutate(
...
print(lesion_qc_df)
cat("\nTotal lesional spots before QC:", sum(lesion_qc_df$before_QC), "\n")
cat("Total lesional spots after QC:", sum(lesion_qc_df$after_QC), "\n")
cat("Total removed:", sum(lesion_qc_df$removed), "\n")
```
...
# Plot stromal topics
pdf(paste0(save_pathL, "3_A_dimplot_integrated_Eth_cohort_splitby_dominant_groupby_stromal_notrastered.pdf"), 
    width = 10, height = 8)
...
# Plot immune topics
pdf(paste0(save_pathL, "3_B_dimplot_integrated_Eth_cohort_splitby_dominant_groupby_immune.pdf"), 
    width = 12, height = 8)
SCpubr::do_DimPlot(integrated_Eth_cohort_filteredL, 
                   split.by = "dominant_celltype", 
...
complete_topic_order <- c(immune_topics, stromal_topics)
