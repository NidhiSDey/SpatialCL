---
title: "P5_L Analysis - Figure 8 and Supplementary Figure S7"
...
# Define paths
read_path <- paste0(study_path, "R/No_P5/test/")
...
# P5 annotation CSV paths
P5_annot_base <- paste0(study_path, "V12N28-391/")
P5_NL_epi_csv <- paste0(P5_annot_base, "PK_391_7_2_run/outs/Keratinocytes.csv")
P5_L_epi_csv <- paste0(P5_annot_base, "PK_391_7_4_run/outs/All_Keratinocyte.csv")
P5_L_subclass_csv <- paste0(P5_annot_base, "PK_391_7_4_run/outs/Keratinocyte_subclass.csv")

# Create save directory for P5
save_pathP5 <- paste0(save_pathL, "P5/test/")
if(!dir.exists(save_pathP5)) {
  dir.create(save_pathP5, recursive = TRUE)
}
```
...
# Load lesion only integrated object (for P5_L vs other lesions comparison)
...
# Load lesion + non-lesion integrated object (for P5_L vs P5_NL comparison)
...
# Set up P5_L comparison
integrated_Eth_cohort_filteredL$P5L_group <- ifelse(
  integrated_Eth_cohort_filteredL$sample_id == "P5_L",
  "P5_L",
  "Other_Patients"
)

cat("\n=== Patient Grouping ===\n")
print(table(integrated_Eth_cohort_filteredL$P5L_group))

Idents(integrated_Eth_cohort_filteredL) <- "P5L_group"
```

# ============================================
# DIFFERENTIAL EXPRESSION: P5_L vs OTHERS
# ============================================
```{r DE_P5_vs_others}
cat("\n=== Running DE: P5_L vs Other Patients ===\n")

P5_vs_others_DE <- FindMarkers(
...
# Add gene names
P5_vs_others_DE$gene <- rownames(P5_vs_others_DE)

# Filter significant
P5_vs_others_sig <- P5_vs_others_DE %>%
  filter(p_val_adj < 0.05) %>%
  arrange(desc(abs(avg_log2FC)))

cat("\n=== P5_L vs Others: Significant DEGs ===\n")
cat("Total significant DEGs (p_adj<0.05):", nrow(P5_vs_others_sig), "\n")
cat("Upregulated in P5_L:", sum(P5_vs_others_sig$avg_log2FC > 0), "\n")
cat("Downregulated in P5_L:", sum(P5_vs_others_sig$avg_log2FC < 0), "\n")

# Save results
write.csv(P5_vs_others_sig, 
          paste0(save_pathP5, "P5_vs_Others_DEGs_significant.csv"),
          row.names = FALSE)
...
# ============================================
# FIGURE 8A: Heatmap P5_L vs Others
# ============================================
```{r figure_8A}
cat("\n=== FIGURE 8A: Heatmap Top 50 DEGs ===\n")

# Select top 25 up and top 25 down
top_up <- P5_vs_others_sig %>%
  filter(avg_log2FC > 0.5) %>%
  arrange(desc(avg_log2FC)) %>%
  head(25)

top_down <- P5_vs_others_sig %>%
  filter(avg_log2FC < -0.5) %>%
  arrange(avg_log2FC) %>%
  head(25)

top_genes <- rbind(top_up, top_down)
top_genes_P5 <- rownames(top_genes)

# Scale data
integrated_Eth_cohort_filteredL <- ScaleData(
  integrated_Eth_cohort_filteredL, 
  features = top_genes_P5
)
...
# Column annotation
col_anno_P5 <- HeatmapAnnotation(
  Patient = integrated_Eth_cohort_filteredL$sample_id,
  Comparison = integrated_Eth_cohort_filteredL$P5L_group,
  col = list(
...
# Create heatmap
pdf(paste0(save_pathP5, "8_A_P5_vs_Others_Heatmap_Top50.pdf"), 
    width = 14, height = 14)
...
ht_P5 <- Heatmap(
  ...
  cluster_rows = TRUE,
  column_split = integrated_Eth_cohort_filteredL$P5L_group,
  row_split = 6,
  column_title = "P5_L vs Other Patients: Top 50 DEGs",
  column_title_gp = gpar(fontsize = 14, fontface = "bold"),
  border = TRUE
)

draw(ht_P5, heatmap_legend_side = "right", annotation_legend_side = "right")
dev.off()
...
# Reorder columns
col_order <- c("P1_L", "P2_L", "P3_L", "P4_L", "P5_L")
ker_patient_matrix <- ker_patient_matrix[, col_order]

write.csv(ker_patient_matrix,
          paste0(save_pathP5, "Keratinocyte_Proportions_AllPatients.csv"))
...
pdf(paste0(save_pathP5, "8_B_Keratinocyte_Heatmap_AllPatients.pdf"),
    width = 8, height = 7)
...
ht_ker <- Heatmap(
  ...
  cell_fun = function(j, i, x, y, width, height, fill) {
    if (colnames(ker_patient_matrix)[j] == "P5_L") {
...
ggsave(paste0(save_pathP5, "8_C_Keratinocyte_InflammedRatio.pdf"),
...
ker_for_stats <- meta_all_patients %>%
  mutate(patient_group = ifelse(sample_id == "P5_L", "P5_L", "Others")) %>%
...
for (ct in keratinocyte_celltypes) {
  ...
  P5_values <- ct_data %>% filter(patient_group == "P5_L") %>% pull(proportion)
  other_values <- ct_data %>% filter(patient_group == "Others") %>% pull(proportion)
  
  wt <- wilcox.test(P5_values, other_values)
  
  ker_enrichment_stats <- rbind(
    ker_enrichment_stats,
    data.frame(
      celltype = ct,
      P5_mean = mean(P5_values, na.rm = TRUE),
      P5_median = median(P5_values, na.rm = TRUE),
      others_mean = mean(other_values, na.rm = TRUE),
      others_median = median(other_values, na.rm = TRUE),
      fold_change = mean(P5_values, na.rm = TRUE) / mean(other_values, na.rm = TRUE),
      p_value = wt$p.value
    )
  )
}
...
# ============================================
# IMPORT P5 EPIDERMIS ANNOTATIONS
# ============================================
```{r import_epidermis_annotations}
cat("\n=== IMPORTING P5 EPIDERMIS ANNOTATIONS ===\n")

# Import annotations
P5_NL_epi_annotation <- read.csv(P5_NL_epi_csv)
P5_L_epi_annotation <- read.csv(P5_L_epi_csv)
P5_L_ker_subclass_anno <- read.csv(P5_L_subclass_csv)

cat("P5_NL epidermis:", sum(P5_NL_epi_annotation$Keratinocytes == "epidermis"), "spots\n")
cat("P5_L total keratinocytes:", sum(P5_L_epi_annotation$All_Keratinocyte != ""), "spots\n")
cat("P5_L subclasses:\n")
print(table(P5_L_ker_subclass_anno$Keratinocyte.subclass))

# Prepare P5_NL annotations
P5NL_epi_barcodes <- P5_NL_epi_annotation %>%
...
# Prepare P5_L annotations
P5l_epi_barcodes <- P5_L_epi_annotation %>%
...
P5l_subclass <- P5_L_ker_subclass_anno %>%
  filter(Keratinocyte.subclass != "") %>%
  select(Barcode, epi_subtype = Keratinocyte.subclass)

P5l_epi_barcodes <- P5l_epi_barcodes %>%
  left_join(P5l_subclass, by = "Barcode")

cat("P5_L epidermis:", nrow(P5l_epi_barcodes), "spots\n")

# Subset P5 samples from combined object
P5_comparison <- subset(integrated_Eth_cohort_filtered,
                        subset = sample_id %in% c("P5_L", "P5_NL"))

cat("\n=== P5 Comparison Subset ===\n")
cat("Total spots:", ncol(P5_comparison), "\n")
print(table(P5_comparison$sample_id))

# Combine annotations
all_epi_barcodes <- bind_rows(
  P5NL_epi_barcodes %>% select(Barcode, epidermis_type, sample_id),
  P5l_epi_barcodes %>% select(Barcode, epidermis_type, sample_id)
)

# Match barcode format
P5_spot_ids <- colnames(P5_comparison)

if (grepl("-1$", P5_spot_ids[1]) && grepl("-1$", all_epi_barcodes$Barcode[1])) {
  all_epi_barcodes$Barcode_full <- all_epi_barcodes$Barcode
} else if (grepl("_\\d+$", P5_spot_ids[1]) && !grepl("_\\d+$", all_epi_barcodes$Barcode[1])) {
  all_epi_barcodes <- all_epi_barcodes %>%
    mutate(
      Barcode_full = case_when(
        sample_id == "P5_L" ~ paste0(Barcode, "_10"),
        sample_id == "P5_NL" ~ paste0(Barcode, "_9"),
        TRUE ~ Barcode
      )
    )
} else {
...
epi_spots_keep <- all_epi_barcodes$Barcode_full[all_epi_barcodes$Barcode_full %in% colnames(P5_comparison)]

P5_epi <- P5_comparison[, epi_spots_keep]
P5_epi$epidermis_type <- all_epi_barcodes$epidermis_type[match(colnames(P5_epi), all_epi_barcodes$Barcode_full)]

cat("\n=== P5 Epidermis Object ===\n")
cat("Total epidermal spots:", ncol(P5_epi), "\n")
print(table(P5_epi$epidermis_type, P5_epi$sample_id))

# Add P5_L subtype info
P5l_spots <- colnames(P5_epi)[P5_epi$sample_id == "P5_L"]
P5_epi$epi_subtype <- NA

for (i in 1:nrow(P5l_epi_barcodes)) {
  bc_full <- if ("Barcode_full" %in% colnames(P5l_epi_barcodes)) {
    P5l_epi_barcodes$Barcode_full[i]
  } else {
    paste0(P5l_epi_barcodes$Barcode[i], "_10")
  }
  
  if (bc_full %in% colnames(P5_epi) && !is.na(P5l_epi_barcodes$epi_subtype[i])) {
    P5_epi$epi_subtype[colnames(P5_epi) == bc_full] <- P5l_epi_barcodes$epi_subtype[i]
  }
}
...
# ============================================
# FIGURE 8D: Spatial P5_L vs P5_NL Epidermis
# ============================================
```{r figure_8D}
...
p1_h_spatial <- SpatialDimPlot(
  ...
  labs(title = "P5_NL: Normal Epidermis")
...
p1_l_spatial <- SpatialDimPlot(
  ...
  labs(title = "P5_L: Acanthotic Epidermis")

p_spatial <- p1_h_spatial + p1_l_spatial

ggsave(paste0(save_pathP5, "8_D_Spatial_P5_Epidermis_LvsH_Annotated.pdf"),
...
Idents(P5_epi) <- "epidermis_type"
P5_epi <- PrepSCTFindMarkers(P5_epi)
...
ggsave(paste0(save_pathP5, "8_E_P5_Acanthotic_DotPlot.pdf"),
...
# ============================================
# P5_L EPIDERMIS SUBCLUSTERING
# ============================================
```{r P5L_epidermis_subclustering}
cat("\n=== P5_L EPIDERMIS SUBCLUSTERING ===\n")

# Subset P5_L from lesional object
P5l_lesional <- subset(integrated_Eth_cohort_filteredL, subset = sample_id == "P5_L")

cat("P5_L lesional spots:", ncol(P5l_lesional), "\n")

# Import P5_L annotations
P5l_annot <- P5_L_epi_annotation %>%
  select(Barcode, epi_region = All_Keratinocyte) %>%
  left_join(
    P5_L_ker_subclass_anno %>% select(Barcode, epi_subtype = Keratinocyte.subclass),
    by = "Barcode"
  )

# Add barcode suffix
P5l_barcodes <- colnames(P5l_lesional)

if (grepl("_10$", P5l_barcodes[1]) && !grepl("_10$", P5l_annot$Barcode[1])) {
  P5l_annot$Barcode_full <- paste0(P5l_annot$Barcode, "_10")
} else {
  P5l_annot$Barcode_full <- P5l_annot$Barcode
}

# Add annotations
P5l_lesional$epi_region <- NA
P5l_lesional$epi_subtype <- NA
P5l_lesional$is_epidermis <- "Non-Epidermis"

for (i in 1:nrow(P5l_annot)) {
  bc <- P5l_annot$Barcode_full[i]
  
  if (bc %in% colnames(P5l_lesional)) {
    if (P5l_annot$epi_region[i] != "") {
      P5l_lesional$epi_region[colnames(P5l_lesional) == bc] <- P5l_annot$epi_region[i]
      P5l_lesional$is_epidermis[colnames(P5l_lesional) == bc] <- "Epidermis"
    }
    
    if (!is.na(P5l_annot$epi_subtype[i]) && P5l_annot$epi_subtype[i] != "") {
      P5l_lesional$epi_subtype[colnames(P5l_lesional) == bc] <- P5l_annot$epi_subtype[i]
    }
  }
}

cat("\n=== Annotation Summary ===\n")
cat("Epidermis spots:", sum(P5l_lesional$is_epidermis == "Epidermis"), "\n")
print(table(P5l_lesional$epi_subtype, useNA = "ifany"))

# Subset to epidermis
P5l_epidermis <- subset(P5l_lesional, subset = is_epidermis == "Epidermis")

cat("Epidermal spots for subclustering:", ncol(P5l_epidermis), "\n")

# Re-process
P5l_epidermis <- FindVariableFeatures(P5l_epidermis, nfeatures = 2000)
P5l_epidermis <- ScaleData(P5l_epidermis)
P5l_epidermis <- RunPCA(P5l_epidermis, npcs = 50, verbose = FALSE)
ElbowPlot(P5l_epidermis)
P5l_epidermis <- RunUMAP(P5l_epidermis, dims = 1:15, verbose = FALSE)
P5l_epidermis <- FindNeighbors(P5l_epidermis, dims = 1:15, verbose = FALSE)

# Clustering at resolution 0.8
P5l_epidermis <- FindClusters(P5l_epidermis, resolution = 0.8, verbose = FALSE)

cat("\n=== Clusters at resolution 0.6 ===\n")
print(table(P5l_epidermis$SCT_snn_res.0.8))
...
write.csv(epi_cluster_markers_sig, 
          paste0(save_pathP5, "epi_cluster_markers_sig.csv"))
...
cluster_names_final <- c(
  ...
  "3" = "Cornified_Layer",
  "4" = "Rete_Ridge_Hyperplasia",
  "5" = "Metabolic"
)

# Get cluster IDs
cluster_ids <- as.character(P5l_epidermis$SCT_snn_res.0.8)
...
# Add to metadata
P5l_epidermis$cluster_name <- cluster_labels

P5l_epidermis$cluster_name <- factor(
  P5l_epidermis$cluster_name,
...
Idents(P5l_epidermis) <- "cluster_name"
...
p_umap_final <- DimPlot(
  ...
  labs(title = "P5_L Epidermis: Unbiased Clustering") +
  theme_void() +
  theme(
...
ggsave(paste0(save_pathP5, "8_F_UMAP_Clusters.pdf"),
...
p_spatial_final <- SpatialDimPlot(
  ...
  labs(title = "P5_L: Epidermal Cluster Spatial Distribution") +
  theme_void() +
  theme(
    plot.title = element_text(face = "bold", size = 16, hjust = 0.5),
    legend.position = "right"
...
ggsave(paste0(save_pathP5, "8_G_Spatial_Clusters.pdf"),
...
all_celltypes_available <- colnames(P5l_epidermis@meta.data)[
  colnames(P5l_epidermis@meta.data) %in% celltypes
]

# Calculate enrichment
celltype_enrichment <- P5l_epidermis@meta.data %>%
...
p_celltype_stacked <- ggplot(celltype_enrichment_filtered,
  ...
  labs(
    title = "P5_L: Non-Keratinocyte Cell Infiltration by Cluster",
...
ggsave(paste0(save_pathP5, "8_H_CellType_Stacked.pdf"),
...
genes_curated_filtered <- lapply(genes_curated, function(gene_vec) {
  gene_vec[gene_vec %in% rownames(P5l_epidermis)]
})

genes_curated_filtered <- genes_curated_filtered[sapply(genes_curated_filtered, length) > 0]

p_dotplot_final <- SCpubr::do_DotPlot(
  sample = P5l_epidermis,
  features = genes_curated_filtered,
  flip = TRUE,
  legend.position = "right",
  plot.title = "P5_L Epidermis: Cluster Signature Genes",
  font.size = 11,
  dot.scale = 8
)

ggsave(paste0(save_pathP5, "S7_A_DotPlot_ClusterMarkers.pdf"),
...
pdf(paste0(save_pathP5, "S7_B_TOP_PANEL_Clusters_Highlighted.pdf"), 12, 10)

cluster_spatial_plots <- SpatialDimPlot(
  P5l_epidermis, 
  cells.highlight = CellsByIdentities(
    object = P5l_epidermis, 
...
coords <- GetTissueCoordinates(P5l_epidermis)
meta <- P5l_epidermis@meta.data
...
p_scatterpie_all <- ggplot() +
  ...
  coord_equal() +
  labs(
    title = "P5_L Epidermis: Cell Type Composition by Cluster",
...
ggsave(paste0(save_pathP5, "S7_B_BOTTOM_PANEL_Scatterpie_AllClusters.pdf"),
...
# Spot-level data
celltype_spot_level <- P5l_epidermis@meta.data %>%
  select(cluster_name, all_of(all_celltypes_available)) %>%
  pivot_longer(cols = -cluster_name, names_to = "celltype", values_to = "proportion")
...
cluster_info$Signature_Genes <- sapply(cluster_info$Cluster_Name, function(name) {
  ...
  genes_present <- genes[genes %in% rownames(P5l_epidermis)]
  paste(genes_present, collapse = ", ")
})

cluster_info$N_Signature_Genes <- sapply(cluster_info$Cluster_Name, function(name) {
  genes <- gene_mapping[[name]]
  sum(genes %in% rownames(P5l_epidermis))
})

cluster_counts <- table(P5l_epidermis$cluster_name)
...
write.csv(ker_enrichment_stats,
          paste0(save_pathP5, "S_Table_5_Keratinocyte_Stats_P5vsOthers.csv"),
          row.names = FALSE)
...
write.csv(de_genes_annotated,
          paste0(save_pathP5, "S_Table_6_Acanthotic_vs_Normal_DEGs_Annotated.csv"),
          row.names = FALSE)
...
write.csv(cluster_table,
          paste0(save_pathP5, "S_Table_7_P5L_Epidermis_Cluster_Summary.csv"),
          row.names = FALSE)
...
write.csv(significant_enrichments,
          paste0(save_pathP5, "S_Table_8_Significant_CellType_Enrichments.csv"),
          row.names = FALSE)
