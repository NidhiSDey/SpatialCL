...
# output: 1. lda file with 8-60 topics: "combined_lda_model_k8_60.Rdata", 2. final Seurat obj with final selected (and combined) 23 topics added: integrated_seurat_obj_23_LDA_topics_added.rds
...
cat("==================================================\n\n")
# First run cleanCounts on each sample separately to get per-sample stats
# Get original spot counts per sample from raw data before QC
original_counts <- table(integrated_Eth_cohort$sample_id)
...
# Spots retained after QC (from your metadata)
# Create comparison dataframe
before <- as.data.frame(table(integrated_Eth_cohort$sample_id))
after  <- as.data.frame(table(integrated_Eth_cohort_filtered$sample_id))

colnames(before) <- c("sample_id", "before_QC")
colnames(after)  <- c("sample_id", "after_QC")

qc_comparison <- before %>%
  left_join(after, by = "sample_id") %>%
  mutate(
...
# Summary
cat("\nTotal spots before QC:", sum(qc_comparison$before_QC), "\n")
cat("Total spots after QC:", sum(qc_comparison$after_QC), "\n")
cat("Total spots removed:", sum(qc_comparison$removed), "\n")
...
output_file <- paste0(save_path, "integrated_seurat_obj_", 
                      n_topics_combined, "_LDA_topics_added.rds")
...
cat("==================================================\n")
cat("\nGenerated Files:\n")
cat("1. LDA model:", paste0("combined_lda_model_k8_60.Rdata\n"))
...
cat("5. Integrated Seurat object:", paste0("integrated_seurat_obj_", 
                      n_topics_combined, "_LDA_topics_added.rds \n"))
