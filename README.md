...
[Space Ranger alignment]
    ↓
Script 1: Integration & Clustering →integrated_seurat_obj.Rds
    ↓
Script 2: STdeconvolve (all samples) → integrated_seurat_obj_23_LDA_topics_added.rds
    ↓
Script 3: STdeconvolve (lesions only) → lesion_only_integrated_obj_35_LDA_topics_added.rds
    ↓
...
|--------|-------------|----------------|--------------------------|
| **Script 1** | Seurat integration, clustering, clinical metadata | Figure 2, Supplementary Figure 1 | `integrated_seurat_obj.Rds` |
| **Script 2** | STdeconvolve deconvolution (all samples: L + NL) | Figure 2, Supplementary Figure 1 | `integrated_seurat_obj_23_LDA_topics_added.rds` |
| **Script 3** | STdeconvolve deconvolution (lesions only) | Figure 3, Supplementary Figure 2, Supplementary Table 2 | `lesion_only_integrated_obj_35_LDA_topics_added.rds` |
...
1. **Lesion-only analysis** (35 STdeconvolve topics):
   - `lesion_only_integrated_obj_35_LDA_topics_added.rds`
...
2. **Combined lesion + non-lesional** (26 topics):
   - `integrated_seurat_obj_23_LDA_topics_added.rds`
...
- **Processed Seurat objects**: Available from Zenodo (https://doi.org/10.5281/zenodo.18471139) or by request to corresponding author
...
seurat_obj <- readRDS("lesion_only_integrated_obj_35_LDA_topics_added.rds")
